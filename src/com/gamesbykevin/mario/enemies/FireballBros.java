...
public final class FireballBros extends Enemy
{
    ...
    private static final double PIXEL_RANGE = Tile.WIDTH * 4;
    
    public FireballBros(final Random random)
    {
        ...
        else
        {
            ...
            if (getSpriteSheet().hasFinished())
            {
                if (canThrowProjectile())
                {
                    ...
                    //set temp location
                    setY(y - BrosFireball.PROJECTILE_HEIGHT);
                    setX(middleX - (BrosFireball.PROJECTILE_WIDTH / 2));
