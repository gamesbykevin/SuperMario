...
import com.gamesbykevin.mario.projectiles.Hammer;

import java.util.Random;

public final class HammerBros extends Enemy
{
    /**
    ...
    private static final double PIXEL_RANGE = Tile.WIDTH * 7;
    
    public HammerBros(final Random random)
    {
    ...
    {
        //walking animation
        super.addAnimation(State.Walking, 2, 0, 534, 16, 24, Timers.toNanoSeconds(250L), true);
        
        //stunned animation
        super.addAnimation(State.Attacking, 2, 32, 534, 16, 24, Timers.toNanoSeconds(650L), false);
    ...
    public void update(final Hero hero, final Level level, final long time)
    {
        ...
        if (!isAnimation(State.Attacking))
        {
            ...
            if (pixelDistance <= PIXEL_RANGE || isAnimation(State.Walking))
            {
        ...
        else
        {
            ...
            if (getSpriteSheet().hasFinished())
            {
                if (canThrowProjectile())
                {
                    ...
                    //set temp location
                    setY(y - Hammer.PROJECTILE_HEIGHT);
                    setX(middleX - (Hammer.PROJECTILE_WIDTH / 2));

                    //add projectile
                    addProjectile(new Hammer((heroMiddleX > middleX) ? true : false));
